...
{
   int iacc,imd,nstep,zero;
   double gamma,eps,lambda;
   char line[NAME_SIZE];
   ...
   zero=0;
   (void)(set_smd_parms(1,&zero,0,0,NULL,1,gamma,eps,iacc));
   (void)(set_mdint_parms(0,(integrator_t)(imd),lambda,nstep,1,&zero));
   (void)(set_action_parms(0,ACG,0,0,NULL,NULL,NULL));
   (void)(set_force_parms(0,FRG,0,0,NULL,NULL,NULL,NULL));
}
...
static void write_schedule(void)
{
   ...
   if (my_rank==0)
   {
         ...
         iw=fwrite(istd,sizeof(stdint_t),3,fdat);
         error_root(iw!=3,1,"write_schedule [ym2.c]",
                    "Incorrect write count");
   }
}
...
static void check_schedule(void)
{
   ...
   {
      ir=fread(istd,sizeof(stdint_t),3,fdat);
      error_root(ir!=3,1,"check_schedule [ym2.c]",
                 "Incorrect read count");

      if (endian==BIG_ENDIAN)
         bswap_int(3,istd);

      ie=(((dnms==0)&&((int)(istd[2])!=0))||
          ((dnms!=0)&&((int)(istd[2])==0)));

      error_root(ie!=0,1,"check_schedule [ym2.c]",
...
static void read_infile(int argc,char *argv[])
{
   ...
   read_lat_parms("Lattice parameters",0x1);
...
static void check_old_log(int ic,int *nl,int *icnfg)
{
   ...
   error_root((ipgrd[0])&&(norng==0),1,"check_old_log [ym2.c]",
              "Continuation run:\n"
              "MPI process grid changed, but -norng is not set");

   error_root((ipgrd[2])&&(norng==0),1,"check_old_log [ym2.c]",
              "Continuation run:\n"
...
static void check_files(int *nl,int *icnfg)
{
   ...
   error(name_size("%sn%d",nbase,icmax)>=NAME_SIZE,1,
         "check_files [ym2.c]","Configuration base name is too long");
   sprintf(cnfg_file,"%sn%d",nbase,icmax);
   check_iodat(iodat+1,"o",0x3,cnfg_file);

   if (scnfg)
   {
      if (append)
         check_iodat(iodat,"i",0x3,cnfg);
      else
...
int main(int argc,char *argv[])
{
   ...
   for (n=0;(iend==0)&&(n<ntot);)
   {
      ...
      if ((n>=nth)&&((n%dncnfg)==0))
      {
         save_flds(icnfg,nbase,iodat+1,0x3);
         ...
         {
            if (rmold)
               remove_flds(icnfg-1,nbase,iodat+1,0x3);
            else
               remove_flds(icnfg-1,nbase,iodat+1,0x2);
         }
