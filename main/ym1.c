...
* File ym1.c
*
* Copyright (C) 2010-2019, 2022 Martin Luescher
*
...
*
* Syntax: ym1 -i <filename> [-c <filename> [-a [-norng]]|[-mask <int>]]
*                           [-rmold] [-noms]
*
...
static void read_integrator(void)
{
   ...
   zero=0;
   (void)(set_hmc_parms(1,&zero,0,0,NULL,1,tau));
...
static void write_schedule(void)
{
   ...
   if (my_rank==0)
   {
      ...
      iw=fwrite(istd,sizeof(stdint_t),3,fdat);
      error_root(iw!=3,1,"write_schedule [ym1.c]",
                 "Incorrect write count");
   }
}
...
static void check_schedule(void)
{
   ...
   {
      ir=fread(istd,sizeof(stdint_t),3,fdat);
      error_root(ir!=3,1,"check_schedule [ym1.c]",
                 "Incorrect read count");
      ...
      error_root(ie!=0,1,"check_schedule [ym1.c]",
...
static void read_infile(int argc,char *argv[])
{
   ...
   if (my_rank==0)
   {
      ...
      if ((scnfg)&&(!append)&&(imask))
      {
         ...
         error_root(ir!=1,1,"read_infile [ym1.c]",
                    "Syntax: ym1 -i <filename> "
                    "[-c <filename> [-a [-norng]]|[-mask <int>]] "
                    "[-rmold] [-noms]");
         error_root((mask<0x0)||(mask>0xf),1,"read_infile [ym1.c]",
...
static void check_old_log(int ic,int *nl,int *icnfg)
{
   ...
   error_root(irg!=0,1,"check_old_log [ym1.c]",
              "Continuation run:\n"
              "Attempt to reuse previously used ranlux level and seed");

   error_root((ipgrd[0])&&(norng==0),1,"check_old_log [ym1.c]",
              "Continuation run:\n"
              "MPI process grid changed, but -norng is not set");

   error_root((ipgrd[2])&&(norng==0),1,"check_old_log [ym1.c]",
              "Continuation run:\n"
...
static void check_files(int *nl,int *icnfg)
{
   ...
   if (my_rank==0)
   {
      ...
      else
      {
         ...
         error_root(ie!=0,1,"check_files [ym1.c]",
   ...
   error(name_size("%sn%d",nbase,icmax)>=NAME_SIZE,1,
         "check_files [ym1.c]","Configuration base name is too long");
