...
static void set_psi(spinor_dble **chi,spinor_dble *psi)
{
   ...
   t=mdtime();
   assign_sd2sd(VOLUME_TRD,2,chi[0],psi);

   for (i=1;i<4;i++)
   {
      z.re=pow(t,(double)(i));
      z.im=0.0;
      mulc_spinor_add_dble(VOLUME_TRD,2,psi,chi[i],z);
   }
}
...
int main(int argc,char *argv[])
{
   ...
   for (i=0;i<4;i++)
      random_sd(VOLUME_TRD,2,chi[i],1.0);
   ...
   for (;s<sm;s++)
   {
      ...
      else if (iop==1)
      {
         ...
         {
            mulr_spinor_add_dble(VOLUME_TRD,2,wsd[1],wsd[0],-1.0);
            rqsm=norm_square_dble(VOLUME_TRD,3,wsd[1]);
            dev=rqsm.q[0];
            rqsm=norm_square_dble(VOLUME_TRD,3,wsd[0]);
            dev/=rqsm.q[0];
