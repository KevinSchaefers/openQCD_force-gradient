...
static qflt random_pf(void)
{
   ...
   wsd=reserve_wsd(1);
   phi=wsd[0];
   random_sd(VOLUME_TRD,2,phi,1.0);
   bnd_sd2zero(ALL_PTS,phi);
   nrm=norm_square_dble(VOLUME_TRD,3,phi);

   mdfs=mdflds();
   assign_sd2sd(VOLUME_TRD,2,phi,(*mdfs).pf[0]);
   release_wsd();
...
static void divide_pf(double mu,int isp,int *status)
{
   ...
   phi=wsd[0];
   mdfs=mdflds();
   assign_sd2sd(VOLUME_TRD,2,(*mdfs).pf[0],phi);
   sp=solver_parms(isp);

   if (sp.solver==CGNE)
   {
      tmcg(sp.nmx,sp.istop,sp.res,mu,phi,phi,ifail,status);

      if (ifail[0]<0)
      {
         print_status("tmcg",ifail,status);
         error_root(1,1,"divide_pf [check5.c]","CGNE solver failed "
   ...
   else if (sp.solver==SAP_GCR)
   {
      ...
      if (ifail[0]<0)
      {
         ...
         error_root(1,1,"divide_pf [check5.c]","SAP_GCR solver failed "
                    "(mu=%.2e, parameter set no %d)",mu,isp);
      }
   }
   else if (sp.solver==DFL_SAP_GCR)
   {
      ...
      if ((ifail[0]<-2)||(ifail[1]<0))
      {
         ...
         error_root(1,1,"divide_pf [check5.c]","DFL_SAP_GCR solver failed "
                    "(mu=%.2e, parameter set no %d)",mu,isp);
      }
   }
      ...
      for (isp=0;isp<3;isp++)
      {
         ...
         if (isp==2)
         {
            ...
            if (ifail[0]<0)
            {
               ...
               error_root(1,1,"main [check5.c]","dfl_modes() failed");
            }
         }
         ...
         if (my_rank==0)
         {
            ...
            else
            {
               ...
               if (isp==0)
                  print_std_status("tmcg","tmcg",status);
               else if (isp==1)
