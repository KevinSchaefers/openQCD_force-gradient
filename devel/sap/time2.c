...
int main(int argc,char *argv[])
{
   ...
   normalize(VOLUME_TRD,3,ps[2]);

   nt=(int)(2.0e6/(double)(ncy*nmr*VOLUME_TRD));
   if (nt<2)
      nt=2;
   wdt=0.0;
   ...
   while (wdt<5.0)
   {
      ...
      for (count=0;count<nt;count++)
      {
         ...
         assign_s2s(VOLUME_TRD,2,ps[2],ps[1]);
   ...
   wdt=2.0e6*wdt/((double)(nt)*(double)(ncy*VOLUME_TRD));

   if (my_rank==0)
   {
      printf("Using the MinRes block solver:\n");
      printf("Time per lattice point & thread:   %.3f micro sec\n",
             (double)(ncy)*wdt);
      printf("Time per point, thread & cycle:    %.3f micro sec",wdt);
