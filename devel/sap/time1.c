...
int main(int argc,char *argv[])
{
   ...
   normalize(VOLUME_TRD,3,ps[0]);
   blk_list(SAP_BLOCKS,&nb,&isw);

   nt=(int)(1.0e7/(double)(nmr*VOLUME_TRD));
   if (nt<2)
      nt=2;
   wdt=0.0;
...
#pragma omp parallel private(k,n,count)
      {
   ...
   if (my_rank==0)
   {
      ...
      printf("Time per lattice point & thread: %.3f micro sec",wdt);
