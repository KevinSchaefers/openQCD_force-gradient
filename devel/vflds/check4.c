...
* License (GPL)
*
* Check of the communication program cpvd_int_bnd().
*
*******************************************************************************/
...
static int chk_ext_bnd(int ieo,complex_dble *v)
{
   int np[4],bc,ofs,vol;
...
int main(int argc,char *argv[])
{
   ...
   if (my_rank==0)
   {
      ...
      printf("\n");
      printf("Check of the communication program cpvd_int_bnd()\n");
      printf("-------------------------------------------------\n\n");
   ...
   for (i=0;i<2;i++)
   {
      random_vd(nvec,0,wv[i],1.0);
      set_field(wv[i]);
      assign_vd2vd(nv,0,wv[i],wv[i+1]);

      cpvd_int_bnd(i,wv[i]);
      mulc_vadd_dble(nv,0,wv[i+1],wv[i],z);
