...
* File check7.c
*
* Copyright (C) 2011-2018, 2020, 2021 Stefan Schaefer, Martin Luescher
*
...
static double check_rotpf(double mu0,double mu1,int ipf,int isp,int *status)
{
   ...
   save_ranlux();
   (void)(setpf2(mu0,mu1,ipf,isp,0,status));
   nrm=unorm_dble(VOLUME_TRD,3,phi);

   restore_ranlux();
   rotpf2(mu0,mu1,ipf,0,isp,0,1.234,-1.234,status);
   dev=unorm_dble(VOLUME_TRD,3,phi)/nrm;
   ...
   restore_ranlux();
   (void)(setpf1(mu0,ipf,0));
   mulr_spinor_add_dble(VOLUME_TRD,2,chi,phi,-1.0);
   dev+=unorm_dble(VOLUME_TRD,3,chi)/nrm;
   ...
   if (my_rank==0)
   {
      ...
      if (bc!=0)
         error_root(sscanf(argv[bc+1],"%d",&bc)!=1,1,"main [check7.c]",
                    "Syntax: check7 [-bc <type>] [-sw <type>]");

      if (is!=0)
         error_root(sscanf(argv[is+1],"%d",&is)!=1,1,"main [check7.c]",
                    "Syntax: check7 [-bc <type>] [-sw <type>]");
   }
   ...
   for (isp=0;isp<3;isp++)
   {
      ...
      if (isp==2)
      {
         ...
         if (ifail[0]<0)
         {
            ...
            error_root(1,1,"main [check7.c]","dfl_modes() failed");
         }
      }
      ...
      act1=action2(mu0,mu1,0,isp,0,0,status);
      error_root((status[0]<0)||(status[1]<0),1,"main [check7.c]",
      ...
      if (my_rank==0)
      {
         ...
         printf("Absolute action difference |setpf2-action2| = %.1e,",
                fabs(dev_act[1]));
         printf(" %.1e (local)\n",dev_act[0]);
         printf("Check of rotpf2 = %.1e\n",dev);
         fflush(flog);
      }

      rot_ud(eps);
      act0=action2(mu0,mu1,0,isp,0,1,status);
      scl_qflt(2.0/3.0,act0.q);
      rot_ud(-eps);

      rot_ud(-eps);
      act1=action2(mu0,mu1,0,isp,0,1,status);
      scl_qflt(-2.0/3.0,act1.q);
      rot_ud(eps);

      rot_ud(2.0*eps);
      act=action2(mu0,mu1,0,isp,0,1,status);
