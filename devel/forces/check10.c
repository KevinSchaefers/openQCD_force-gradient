...
static double check_rotpf(double mu,int ipf,int *status)
{
   double nrm;
   spinor_dble *phi;
   mdflds_t *mdfs;
   ...
   save_ranlux();
   (void)(setpf4(mu,ipf,0,0));
   nrm=unorm_dble(VOLUME_TRD/2,3,phi);

   restore_ranlux();
   rotpf4(mu,ipf,0,0,1.234,-1.234,status);

   return unorm_dble(VOLUME_TRD/2,3,phi)/nrm;
}


static qflt dSdt(double mu,int ipf,int isw,int isp,int *status)
{
   mdflds_t *mdfs;
...
int main(int argc,char *argv[])
{
   ...
   int isp,isw,isap,idfl;
   double mu,dev,eps,*qact[1];
   ...
   if (my_rank==0)
   {
      ...
      if (bc!=0)
         error_root(sscanf(argv[bc+1],"%d",&bc)!=1,1,"main [check10.c]",
                    "Syntax: check10 [-bc <type>] [-sw <type>]");

      if (is!=0)
         error_root(sscanf(argv[is+1],"%d",&is)!=1,1,"main [check10.c]",
                    "Syntax: check10 [-bc <type>] [-sw <type>]");
   }
      ...
      for (isp=0;isp<3;isp++)
      {
         ...
         if (isp==2)
         {
            ...
            if (ifail[0]<0)
            {
               ...
               error_root(1,1,"main [check10.c]","dfl_modes() failed");
            }
         }

         if (isw==0)
            dev=check_rotpf(mu,0,status);
         else
            dev=0.0;
         act0=setpf4(mu,0,isw,0);
         act1=action4(mu,0,isw,isp,0,0,status);
         act.q[0]=-act1.q[0];
         act.q[1]=-act1.q[1];
         ...
         if (my_rank==0)
         {
            ...
            printf(" %.1e (local)\n",dev_act[0]);
            if (isw==0)
               printf("Check of rotpf4 = %.1e\n",dev);
            fflush(flog);
         }

         rot_ud(eps);
         act0=action4(mu,0,isw,isp,0,1,status);
         scl_qflt(2.0/3.0,act0.q);
         rot_ud(-eps);

         rot_ud(-eps);
         act1=action4(mu,0,isw,isp,0,1,status);
         scl_qflt(-2.0/3.0,act1.q);
         rot_ud(eps);

         rot_ud(2.0*eps);
         act=action4(mu,0,isw,isp,0,1,status);
