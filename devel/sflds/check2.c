...
* File check2.c
*
* Copyright (C) 2013-2016, 2018, 2021 Martin Luescher
*
...
static void set_sd(spinor_dble *s)
{
   ...
   if (bc==0)
      ...
      p[0]=2.0*pi*(double)(np[0])/(double)(N0);
   else
      p[0]=pi*(double)(np[0])/(double)(N0);

   p[1]=2.0*pi*(double)(np[1])/(double)(N1);
   p[2]=2.0*pi*(double)(np[2])/(double)(N2);
   p[3]=2.0*pi*(double)(np[3])/(double)(N3);
...
int main(int argc,char *argv[])
{
   ...
   for (is=0;is<2;is++)
   {
      for (k=0;k<NFLDS;k+=2)
      {
         random_sd(NSPIN,0,psd[k],1.0);
         assign_sd2s(NSPIN,0,psd[k],ps[k]);
         d=(double)(norm_square(NSPIN,1,ps[k]));
         cps_int_bnd(is,ps[k]);
         cpsd_int_bnd(is,psd[k]);
         assign_sd2s(NSPIN,0,psd[k],ps[k+1]);
         mulr_spinor_add(NSPIN,0,ps[k],ps[k+1],-1.0f);
   ...
   {
      set_sd(psd[0]);
      random_sd(NSPIN-VOLUME,0,psd[0]+VOLUME,1.0);
      assign_sd2sd(VOLUME,0,psd[0],psd[1]);
      cpsd_int_bnd(is,psd[0]);
      mulr_spinor_add_dble(VOLUME,0,psd[1],psd[0],-1.0);
   ...
   for (is=0;is<2;is++)
   {
      random_sd(NSPIN,0,psd[0],1.0);
      set_sd_bnd(is,psd[0]);
      assign_sd2sd(NSPIN,0,psd[0],psd[1]);
      cpsd_ext_bnd(is,psd[0]);
      mulr_spinor_add_dble(NSPIN-VOLUME,0,psd[1]+VOLUME,psd[0]+VOLUME,-1.0);
      rqsm=norm_square_dble(NSPIN-VOLUME,1,psd[1]+VOLUME);
      d=rqsm.q[0];
      if (d>dmax)
         dmax=d;
      mulr_spinor_add_dble(VOLUME,0,psd[0],psd[1],-1.0);
