...
* File time3.c
*
* Copyright (C) 2011-2019, 2021 Martin Luescher
*
...
#pragma omp parallel private(k,count,n)
   ...
   if (my_rank==0)
   {
      printf("Time per lattice point & thread for Dw_blk():\n");
...
#pragma omp parallel private(k,count,n)
   ...
   if (my_rank==0)
   {
      printf("Time per lattice point & thread for Dwhat_blk():\n");
      printf("%4.3f micro sec (%d Mflops)\n\n",wdt,(int)(1908.0/wdt));
      fclose(flog);
   }
