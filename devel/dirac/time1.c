...
int main(int argc,char *argv[])
{
   ...
   for (i=0;i<nflds;i++)
      random_s(VOLUME_TRD,2,ps[i],1.0f);

   nt=(int)(1.0e6f/(double)(nflds*VOLUME_TRD));
   if (nt<2)
      nt=2;
   wdt=0.0;
   ...
   if (my_rank==0)
   {
      printf("Time per lattice point & thread for Dw():\n");
   ...
   if (my_rank==0)
   {
      printf("Time per lattice point &thread for Dwhat():\n");
      printf("%4.3f micro sec (%d Mflops)\n\n",wdt,(int)(1908.0/wdt));
      fclose(flog);
   }
