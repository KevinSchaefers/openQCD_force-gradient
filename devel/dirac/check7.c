...
int main(int argc,char *argv[])
{
   ...
   for (itm=0;itm<2;itm++)
   {
      ...
      for (ic=0;ic<2;ic++)
      {
         random_s(VOLUME_TRD,2,ps[0],1.0f);
         random_s(VOLUME_TRD,2,ps[2],1.0f);
         ...
         mulr_spinor_add(VOLUME_TRD,2,ps[0],ps[2],-1.0f);
         mulr_spinor_add(VOLUME_TRD,2,ps[1],ps[3],-1.0f);

         if (norm_square(VOLUME_TRD,2,ps[0])!=0.0f)
            ie=1;

         d=norm_square(VOLUME_TRD,3,ps[1])/
            norm_square(VOLUME_TRD,3,ps[3]);
      ...
      for (n=0;n<nb;n++)
      {
         ...
         mulr_spinor_add(vol,0,b[n].s[0],b[n].s[1],-1.0f);
      ...
      for (n=0;n<nb;n++)
      {
         ...
         mulr_spinor_add(volh,0,b[n].s[0],b[n].s[2],-1.0f);
         d=norm_square(volh,0,b[n].s[0]);
         if (d>dmax)
            dmax=d;

         assign_s2sblk(SAP_BLOCKS,n,ALL_PTS,ps[0],0);
         mulr_spinor_add(volh,0,b[n].s[0]+volh,b[n].s[1]+volh,-1.0f);
