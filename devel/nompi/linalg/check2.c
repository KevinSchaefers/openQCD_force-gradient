...
int main(void)
{
   ...
   a1=amalloc(2*NMAX*(3*NMAX+2)*sizeof(*a1),5);
   ...
   d7=0.0;
   kmax=0.0;
   cwsp=alloc_cmat_wsp(NMAX);
   error(cwsp==NULL,1,"main [check2.c]",
         "Allocation of cmat_wsp_t workspace failed");
   ...
   for (n=1;n<=NMAX;n++)
   {
      ...
      ie=cmat_inv_dble(n,a1,cwsp,b1,&k1);
      mmul(n,a2,b1,b2);
      mmul(n,a2,b2,c2);
