...
static float (*xs)[NXS],(*ys)[NXS],xsn[96];
static double (*xd)[NXD],(*yd)[NXD],xdn[48];


static void update_xa(int n)
{
   int m,k;
#if (defined _OPENMP)
...
int main(void)
{
   ...
   printf("Direct check of ranlxs() and ranlxd()\n");
   printf("-------------------------------------\n\n");

#if (defined _OPENMP)
   printf("Number of OpenMP threads = ");
   k=scanf("%d",&n);
   error((k!=1)||(n<1),1,"main [check1.c]",
         "Read error or thread number less than 1");
   omp_set_num_threads(n);
#else
   n=1;
#endif

   xs=malloc(2*n*sizeof(*xs));
   xd=malloc(2*n*sizeof(*xd));

   error((xs==NULL)||(xd==NULL),1,"main [check1.c]",
         "Unable to allocate auxiliary arrays");

   ys=xs+n;
   yd=xd+n;

   rlxs_init(n,0,32767,1);
   rlxd_init(n,1,32767,1);
 ...
 *******************************************************************************/
   ...
   for (l=0;l<n;l++)
   {
      rlxs_init(n,0,32767-l,1);
      rlxd_init(n,1,32767-l,1);
...
#pragma omp parallel
      ...
      for (k=0;k<96;k++)
      {
         if (xsn[k]!=(xs[l][k+60]*sbase))
            test1|=1;
      }

      for (k=0;k<48;k++)
      {
         if (xdn[k]!=(xd[l][k+39]*base))
            test2|=1;
      }
   }
...
#pragma omp parallel
   ...
   if (test3)
   {
      printf("\n");
      printf("Test failed: ");
      printf("I/O routines for ranlxs do not work properly\n");
      printf("=> do not use ranlxs on this machine\n");
      printf("\n");
   }
   ...
   if (test4)
   {
      printf("\n");
      printf("Test failed: ");
      printf("I/O routines for ranlxd do not work properly\n");
      printf("=> do not use ranlxd on this machine\n");
      printf("\n");
   }
