...
int main(int argc,char *argv[])
{
   ...
   for (icnfg=first;icnfg<=last;icnfg+=step)
   {
      ...
      for (i=0;i<ntm;i++)
      {
         ...
         {
            if (i3d)
               sphere3d_msfld(rmin+j,f[1]);
            else
               sphere_msfld(rmin+j,f[1]);

            convolute_msfld(NULL,f[0],f[1],rf[0],rf[1],f[1]);
            mul_msfld(f[1],f[0]);
            av[i][j]=center_msfld(f[1]);
            convolute_msfld(NULL,f[1],f[1],rf[0],rf[0],f[1]);
            mulr_msfld(rv,f[1]);
