...
static int ipt_blk(block_t *b,int *x)
{
   ...
   {
      error_loc(1,1,"ipt_blk [check5.c]","Point coordinates are out of range");
      return 0;
   }
}
...
int main(int argc,char *argv[])
{
   ...
   for (n=0;n<nb;n++)
   {
      for (iset=0;iset<=(int)(PT_SETS);iset++)
      {
         ...
         random_s(vol,0,(*b).s[k],0.1f);
         assign_s2s(vol,0,(*b).s[k],(*b).s[l]);
   ...
   for (n=0;n<nb;n++)
   {
      for (iset=0;iset<(int)(PT_SETS);iset++)
      {
         ...
         random_sd(vol,0,(*b).sd[k],0.1f);
         assign_sd2sd(vol,0,(*b).sd[k],(*b).sd[l]);
   ...
   random_s(VOLUME_TRD,2,ps[0],1.0f);
   assign_s2s(VOLUME_TRD,2,ps[0],ps[1]);
   assign_s2sd(VOLUME_TRD,2,ps[0],psd[0]);
