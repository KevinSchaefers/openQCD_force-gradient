...
*
*   cmat_wsp_t *alloc_cmat_wsp(int nmx)
*     Returns the address of a new instance of a cmat_wsp_t workspace for
*     inversions of matrices of size up to nmx x nmax. A NULL pointer is
*     returned if the allocation fails.
*
*   void free_cmat_wsp(cmat_wsp_t *cwsp)
*     Frees the workspace cwsp previously allocated by alloc_cmat_wsp().
*
*   int cmat_inv_dble(int n,complex_dble *a,cmat_wsp_t *cwsp,
...
*     A non-zero return value indicates that the program failed because
*     the matrix a was found be singular within rounding errors. It is up
*     to the user to ensure that the size (*cwsp).nmx of the workspace
*     is greater or equal to n.
*
* Complex nxn matrices with matrix elements A_{ij} are represented by linear
* arrays a of complex numbers such that
*
*   A_{ij} = a[i*n+j]
*
* where i,j=0,1,..,n-1. It is assumed that the complex arrays, on which
* the programs act, do not overlap in memory (the results are otherwise
* unpredictable).
*
...
* AVX is used).
*
* All programs in this module are thread-safe.
*
*******************************************************************************/
...
static void cmat_transpose(int n,complex_dble *a)
{
   int i,j;
   complex_dble z;
...
cmat_wsp_t *alloc_cmat_wsp(int nmx)
{
   ...
   cwsp=malloc(sizeof(*cwsp));
   r=malloc((nmx+8)*sizeof(*r));
   d=malloc((nmx+4)*sizeof(*d));

   if ((cwsp==NULL)||(r==NULL)||(d==NULL))
      return NULL;
...
static int fwd_house(int n,complex_dble *a,cmat_wsp_t *cwsp,
                     complex_dble *b,double *fnsq)
{
   int i,j,k;
   double eps,r1,r2,r3,*rsv;
   complex_dble z,*bb,*bm,*bk,*bj,*dsv;
...
static void solv_sys(int n,cmat_wsp_t *cwsp,complex_dble *b)
{
   int i,j,k;
   complex_dble *bi,*bk,*dsv,z;
...
static void bck_house(int n,cmat_wsp_t *cwsp,complex_dble *b)
{
   int i,j,k;
   double *rsv;
   complex_dble *bi,*dj,*dsv,z;
