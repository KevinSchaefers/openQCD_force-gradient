...
* File salg.c
*
* Copyright (C) 2005-2016, 2021 Martin Luescher
*
...
*
*   void mulc_spinor_add(int vol,int icom,spinor *s,spinor *r,complex z)
*     Replaces the field s by s+z*r.
*
*   void mulr_spinor_add(int vol,int icom,spinor *s,spinor *r,float c)
*     Replaces the field s by s+c*r.
*
*   void scale(int vol,int icom,float c,spinor *s)
*     Replaces the field s by c*s.
*
*   void project(int vol,int icom,spinor *s,spinor *r)
*     Replaces the field s by s-(r,s)*r.
*
*   float normalize(int vol,int icom,int icom,spinor *s)
...
*
*   void mulg5(int vol,int icom,spinor *s)
*     Multiplies the field s with gamma_5.
*
*   void mulmg5(int vol,int icom,spinor *s)
*     Multiplies the field s with -gamma_5.
*
...
static complex_dble loc_spinor_prod(int vol,spinor *s,spinor *r)
{
   complex_dble v;
   spinor *sm;
...
static double loc_spinor_prod_re(int vol,spinor *s,spinor *r)
{
   double x;
   spinor *sm;
...
static double loc_norm_square(int vol,spinor *s)
{
   double x;
   spinor *sm;
...
static void loc_mulc_spinor_add(int vol,spinor *s,spinor *r,complex z)
{
   spinor *sm;
...
static void loc_mulr_spinor_add(int vol,spinor *s,spinor *r,float c)
{
   spinor *sm;
...
static void loc_scale(int vol,float c,spinor *s)
{
   spinor *sm;
...
static void loc_mulg5(int vol,spinor *s)
{
   spinor *sm;
...
static void loc_mulmg5(int vol,spinor *s)
{
   spinor *sm;
...
#pragma omp parallel private(k) reduction(sum_complex_dble : v)
   ...
   if ((NPROC>1)&&(icom&0x1))
   {
      ...
      MPI_Reduce(&w.re,&v.re,2,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
      MPI_Bcast(&v.re,2,MPI_DOUBLE,0,MPI_COMM_WORLD);
   }
...
#pragma omp parallel private(k) reduction(+ : x)
      {
         k=omp_get_thread_num();
         x=loc_spinor_prod_re(vol,s+k*vol,r+k*vol);
      }
   }
...
void mulc_spinor_add(int vol,int icom,spinor *s,spinor *r,complex z)
{
   ...
   if ((icom&0x2)==0)
      loc_mulc_spinor_add(vol,s,r,z);
   else
   {
#pragma omp parallel private(k)
      {
         k=omp_get_thread_num();
         loc_mulc_spinor_add(vol,s+k*vol,r+k*vol,z);
      }
   }
}
...
void mulr_spinor_add(int vol,int icom,spinor *s,spinor *r,float c)
{
   ...
   if ((icom&0x2)==0)
      loc_mulr_spinor_add(vol,s,r,c);
   else
   {
#pragma omp parallel private(k)
      {
         k=omp_get_thread_num();
         loc_mulr_spinor_add(vol,s+k*vol,r+k*vol,c);
      }
   }
}
...
void project(int vol,int icom,spinor *s,spinor *r)
{
   ...
   z.re=-z.re;
   z.im=-z.im;
   mulc_spinor_add(vol,icom,s,r,z);
}
