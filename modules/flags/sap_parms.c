...
* File sap_parms.c
*
* Copyright (C) 2009, 2010, 2011, 2021 Martin Luescher
*
...
*
*   void read_sap_parms(char *section,int rflg)
*     Reads the SAP parameters on MPI process 0 from the specified
...
*       ncy          <int>
*
*     The block sizes bs are read if the bit (rflg&0x1) is set and
*     the other parameters if the bit (rflg&0x2) is set.
*
...
sap_parms_t set_sap_parms(int *bs,int isolv,int nmr,int ncy)
{
   ...
   if (sap.ncy!=0)
   {
      error_root((bs[0]!=sap.bs[0])||(bs[1]!=sap.bs[1])||
                 ...
                 "set_sap_parms [sap_parms.c]","bs may be set only once");
   }
   else
   {
      check_block_size(bs);
...
void read_sap_parms(char *section,int rflg)
{
   int my_rank,bs[4],isolv,nmr,ncy;
