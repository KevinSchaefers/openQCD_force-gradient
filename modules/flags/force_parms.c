...
* File force_parms.c
*
* Copyright (C) 2011, 2012, 2017, 2021 Martin Luescher
*
...
*     (see the notes).
*
*   void read_force_parms(int ifr,int rflg)
*     Reads the parameter section [Force <int>] on MPI process 0 from
*     stdin, where <int> is set to the force index ifr. Depending on
*     the type of force, only a subset of the tags
*
*       force   <force_t>
*       ipf     <int>
*       im0     <int>
...
*       ncr     <int> [<int>]
*
*     must be present in the section. If the bit (rflg&0x1) is set, only
*     the force, isp and ncr tags must be present, all other parameters
*     being inferred from the parameters of the action no ifr (an error
*     occurs if the parameters of the action have not previously been
*     entered to the data base or if the force and action do not match).
*
*   void read_all_force_parms(void)
*     Reads the parameters of all forces associated with the actions
*     listed in the SMD or HMC parameter data base from stdin on MPI
*     process 0. An error occurs if both lists of actions are empty
*     or if the action parameters have not previously been entered in
*     the data base.
*
*   void print_force_parms(int ipr)
*     Prints the parameters of the defined forces to stdout on MPI
*     process 0. Only the parameters force, isp and ncr are printed
*     if ipr=0.
*
...
static void read_force_parms0(int ifr)
{
   int my_rank,i,idf;
   ...
   if (my_rank==0)
   {
      ...
      {
         idf=(int)(FORCES);
         error_root(1,1,"read_force_parms0 [force_parms.c]",
...
static void read_force_parms1(int ifr)
{
   ...
   {
      ap=action_parms(ifr);
      error_root(ap.action==ACTIONS,1,"read_force_parms1 [force_parms.c]",
                 "Undefined action");
      ...
      {
         ie=1;
         error_root(1,1,"read_force_parms1 [force_parms.c]",
                    "Unknown action");
      }

      error_root(ie!=0,1,"read_force_parms1 [force_parms.c]",
...
{
   if (rflg&0x1)
      read_force_parms1(ifr);
   else
      read_force_parms0(ifr);
}
...
void read_all_force_parms(void)
{
   ...
   error_root((hmc.nact==0)&&(smd.nact==0),1,
              "read_all_force_parms [force_parms.c]","Empty action lists");
