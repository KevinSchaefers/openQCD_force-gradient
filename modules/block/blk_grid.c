...
* File blk_grid.c
*
* Copyright (C) 2005, 2007, 2011-2019, 2021 Martin Luescher
*
...
* must satisfy certain constraints (see README.block).
*
...
* All blocks have allocated boundaries and the protection flag is set.
*
* The program alloc_bgr() is assumed to be called by the OpenMP master thread
* on all MPI processes simultaneously. The program blk_list() is thread-safe.
*
*******************************************************************************/
...
{
   int k,ieo,ib,ie;
   int n0,n1,n2,n3,m0,m1,m2,m3,bo[4];
   block_t *b;
...
#pragma omp parallel private(k,ieo,ib,m0,m1,m2,m3,bo) reduction(| : ie)
   {
               ...
               for (m2=0;m2<n2;m2++)
               {
                  for (m3=0;m3<n3;m3++)
                  {
                     if (((m0+m1+m2+m3)&0x1)==ieo)
                     {
                        ...
                        if ((ib==k)&&(ieo==0))
                        {
                           ie|=alloc_blk(b+k,bo,bs,iu,iud,ns,nsd);
                           if (ie==0)
                              ie|=alloc_bnd(b+k,iub,iudb,nw,nwd);
                           b[k].shf=shf&0x1;
                        }
                        else if (((ib%NTHREAD)==k)&&(ie==0))
                           ie|=clone_blk(b+k,shf,bo,b+ib);
   ...
   error(ie!=0,1,"blks [blk_grid.c]","Block grid allocation failed");
