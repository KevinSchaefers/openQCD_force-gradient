...
static void set_ofs(void)
{
   ...
   error(ipt==NULL,1,"set_ofs [scom.c]",
...
static void alloc_sbufs(void)
{
   ...
   w=amalloc(4*n*sizeof(*w),ALIGN);
   error(w==NULL,1,"alloc_sbufs [scom.c]",
...
static void set_requests(void)
{
   ...
   for (ifc=0;ifc<8;ifc++)
   {
      ...
      if (sflg[ifc])
      {
         ...
         MPI_Send_init(sbufs[sfc&0x1],12*nfc[ifc],MPI_FLOAT,saddr,
                       tag,MPI_COMM_WORLD,&snd_req[ifc]);
         MPI_Recv_init(rbufs[sfc&0x1],12*nfc[ifc],MPI_FLOAT,raddr,
...
static void set_bnd_s2zero(int ofs,int vol,spinor *s)
{
   int *pt,*ptm;
...
#pragma omp parallel private(k,ifc,sfc,ofs,vol)
      {
         ...
         if (sflg[sfc])
         {
            ofs=ofs_get[k][0][0];
            vol=ofs_get[k][0][1];
            assign_s2w[sfc^is](map+pofs[sfc^0x1]+ofs,vol,s,
                               sbufs[ifc&0x1]+ofs);
         }
         else
         {
            ...
            if ((NTHREAD==1)||(k>0))
            {
               ...
               if (ifc>1)
               {
                  ...
                  if (sflg[sfc])
                  {
                     ...
                     unzip_weyl(vol,rbufs[ifc&0x1]+ofs,sb+pofs[sfc]+ofs);
                  }
                  ...
                  if ((ifc==2)&&(sflg[0]==1)&&(sflg[1]==0)&&(bc!=0))
                  {
                     ofs=ofs_get[k][1][0];
                     vol=ofs_get[k][1][1];
                     set_s2zero(vol,0,sb+pofs[1]+ofs);
                  }
               }
            }
         }
#pragma omp barrier
         ...
         if (sflg[sfc])
         {
            ofs=ofs_put[k][7][0];
            vol=ofs_put[k][7][1];
            unzip_weyl(vol,rbufs[1]+ofs,sb+pofs[sfc]+ofs);
         }
      }
   }
   else
...
#pragma omp parallel private(k,ifc,sfc,ofs,vol)
      {
         ...
         if (sflg[sfc])
         {
            ofs=ofs_put[k][7][0];
            vol=ofs_put[k][7][1];
            zip_weyl(vol,sb+pofs[sfc]+ofs,sbufs[ifc&0x1]+ofs);
         }
            ...
            if ((NTHREAD==1)||(k>0))
            {
               ...
               if (ifc<6)
               {
                  ...
                  if (sflg[sfc])
                  {
                     ...
                     add_assign_w2s[(sfc^0x1)^is](map+pofs[sfc^0x1]+ofs,vol,
                                                  rbufs[(ifc+2)&0x1]+ofs,s);
                  }
               }
            }
         }
