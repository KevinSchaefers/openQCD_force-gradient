...
* File sflds.c
*
* Copyright (C) 2005, 2011, 2021 Martin Luescher
*
...
*
*   void set_s2zero(int vol,int icom,spinor *s)
*     Sets the single-precision spinor field s to zero.
*
*   void set_sd2zero(int vol,int icom,spinor_dble *sd)
*     Sets the double-precision spinor field sd to zero.
*
*   void random_s(int vol,int icom,spinor *s,float sigma)
*     Initializes the components of the single-precision field s
*     to (complex) random values z with distribution proportional
*     to exp{-|z|^2/sigma^2}.
*
*   void random_sd(int vol,int icom,spinor_dble *sd,double sigma)
*     Initializes the components of the double-precision field sd
*     to (complex) random values z with distribution proportional
*     to exp{-|z|^2/sigma^2}.
*
*   void assign_s2s(int vol,int icom,spinor *s,spinor *r)
*     Assigns the single-precision field s to the single-precision
*     field r.
*
*   void assign_s2sd(int vol,int icom,spinor *s,spinor_dble *rd)
*     Assigns the single-precision field s to the double-precision
*     field rd.
*
*   void assign_sd2s(int vol,int icom,spinor_dble *sd,spinor *r)
*     Assigns the double-precision field sd to the single-precision
*     field r.
*
*   void assign_sd2sd(int vol,int icom,spinor_dble *sd,spinor_dble *rd)
*     Assigns the double-precision field sd to the double-precision
*     field rd.
*
*   void add_s2s(int vol,int icom,spinor *s,spinor *r)
*     Adds the single-precision field s to the single-precision field r.
*
*   void add_s2sd(int vol,int icom,spinor *s,spinor_dble *rd)
*     Adds the single-precision field s to the double-precision field rd.
*
*   void diff_s2s(int vol,int icom,spinor *s,spinor *r)
*     Assigns the difference s-r of the single-precision fields s and r
*     to r.
*
*   void diff_sd2s(int vol,int icom,spinor_dble *sd,spinor_dble *rd,spinor *r)
...
*
* Since no MPI communications are performed, all programs in this file can
* be called locally. If SSE instructions are used, the fields must be aligned
...
static void loc_set_s2zero(int vol,spinor *s)
{
   spinor *sm;
...
static void loc_set_sd2zero(int vol,spinor_dble *sd)
{
   spinor_dble *sm;
...
static void loc_assign_s2s(int vol,spinor *s,spinor *r)
{
   spinor *rm;
...
static void loc_assign_s2sd(int vol,spinor *s,spinor_dble *rd)
{
   spinor_dble *rm;
...
static void loc_assign_sd2s(int vol,spinor_dble *sd,spinor *r)
{
   spinor *rm;
...
static void loc_assign_sd2sd(int vol,spinor_dble *sd,spinor_dble *rd)
{
   spinor_dble *rm;
...
static void loc_add_s2s(int vol,spinor *s,spinor *r)
{
   spinor *sm;
...
static void loc_add_s2sd(int vol,spinor *s,spinor_dble *rd)
{
   spinor_dble *rm;
...
static void loc_diff_s2s(int vol,spinor *s,spinor *r)
{
   spinor *sm;
...
static void loc_diff_sd2s(int vol,spinor_dble *sd,spinor_dble *rd,spinor *r)
{
   spinor *rm;
...
static void loc_add_s2s(int vol,spinor *s,spinor *r)
{
   ...
   for (;s<sm;s++)
   {
      _vector_add_assign((*r).c1,(*s).c1);
      _vector_add_assign((*r).c2,(*s).c2);
      _vector_add_assign((*r).c3,(*s).c3);
      _vector_add_assign((*r).c4,(*s).c4);
...
static void loc_random_s(int vol,spinor *s,float sigma)
{
   float r[24];
   spinor *sm;
...
static void loc_random_sd(int vol,spinor_dble *sd,double sigma)
{
   double r[24];
   spinor_dble *sm;
...
#pragma omp parallel private(k)
      {
         ...
         loc_set_s2zero(vol,s+k*vol);
      }
   }
   else
      loc_set_s2zero(vol,s);
}
...
#pragma omp parallel private(k)
      {
         k=omp_get_thread_num();
         loc_set_sd2zero(vol,sd+k*vol);
      }
   }
   else
      loc_set_sd2zero(vol,sd);
}
...
#pragma omp parallel private(k)
      {
         k=omp_get_thread_num();
         loc_random_s(vol,s+k*vol,sigma);
      }
   }
   else
      loc_random_s(vol,s,sigma);
}
...
#pragma omp parallel private(k)
      {
         k=omp_get_thread_num();
         loc_random_sd(vol,sd+k*vol,sigma);
      }
   }
   else
      loc_random_sd(vol,sd,sigma);
}
...
#pragma omp parallel private(k)
      {
         k=omp_get_thread_num();
         loc_assign_s2s(vol,s+k*vol,r+k*vol);
      }
   }
   else
      loc_assign_s2s(vol,s,r);
}
...
#pragma omp parallel private(k)
      {
         k=omp_get_thread_num();
         loc_assign_s2sd(vol,s+k*vol,rd+k*vol);
      }
   }
   else
      loc_assign_s2sd(vol,s,rd);
}
...
#pragma omp parallel private(k)
      {
         k=omp_get_thread_num();
         loc_assign_sd2s(vol,sd+k*vol,r+k*vol);
      }
   }
   else
      loc_assign_sd2s(vol,sd,r);
}
...
#pragma omp parallel private(k)
      {
         k=omp_get_thread_num();
         loc_assign_sd2sd(vol,sd+k*vol,rd+k*vol);
      }
   }
   else
      loc_assign_sd2sd(vol,sd,rd);
}
...
#pragma omp parallel private(k)
      {
         k=omp_get_thread_num();
         loc_add_s2s(vol,s+k*vol,r+k*vol);
      }
   }
   else
      loc_add_s2s(vol,s,r);
...
#pragma omp parallel private(k)
      {
         k=omp_get_thread_num();
         loc_add_s2sd(vol,s+k*vol,rd+k*vol);
      }
   }
   else
      loc_add_s2sd(vol,s,rd);
}
...
#pragma omp parallel private(k)
      {
         k=omp_get_thread_num();
         loc_diff_s2s(vol,s+k*vol,r+k*vol);
      }
   }
   else
      loc_diff_s2s(vol,s,r);
}
...
#pragma omp parallel private(k)
      {
         k=omp_get_thread_num();
         loc_diff_sd2s(vol,sd+k*vol,rd+k*vol,r+k*vol);
      }
   }
   else
      loc_diff_sd2s(vol,sd,rd,r);
}
