...
* File rwtmeo.c
*
* Copyright (C) 2012-2014, 2018, 2022 Martin Luescher, Stefan Schaefer
*
...
* mass mu1 and sqrt(2)*mu2 [in the case of rwtm2eo()] are written to the
...
qflt rwtm1eo(double mu1,double mu2,int isp,qflt *sqn,int *status)
{
   ...
   if (sp.solver==CGNE)
   {
      tmcgeo(sp.nmx,sp.istop,sp.res,mu1,eta,phi,ifail0,stat0);
      acc_std_status("tmcgeo",ifail0,stat0,0,status);

      if (ifail0[0]<0)
      {
         print_status("tmcgeo",ifail0,stat0);
         error_root(1,1,"rwtm1eo [rwtmeo.c]","CGNE solver failed "
                    "(mu = %.2e, parameter set no %d)",mu1,isp);
      }

      lnr=spinor_prod_re_dble(VOLUME_TRD/2,3,eta,phi);
   }
   else if (sp.solver==SAP_GCR)
   {
      ...
      if (ifail0[0]<0)
      {
         ...
         error_root(1,1,"rwtm1eo [rwtmeo.c]","SAP_GCR solver failed "
                    "(mu = %.2e, parameter set no %d)",mu1,isp);
      }

      lnr=norm_square_dble(VOLUME_TRD/2,3,phi);
   }
   else if (sp.solver==DFL_SAP_GCR)
   {
      ...
      if ((ifail0[0]<-2)||(ifail0[1]<0))
      {
         ...
         error_root(1,1,"rwtm1eo [rwtmeo.c]","DFL_SAP_GCR solver failed "
                    "(mu = %.2e, parameter set no %d)",mu1,isp);
      }
...
qflt rwtm2eo(double mu1,double mu2,int isp,qflt *sqn,int *status)
{
   ...
   if (sp.solver==CGNE)
   {
      ...
      {
         print_status("tmcgeo",ifail0,stat0);
         error_root(1,1,"rwtm2eo [rwtmeo.c]","CGNE solver failed "
                    "(mu = %.2e, parameter set no %d)",mu1,isp);
      }

      tmcgeo(sp.nmx,sp.istop,sp.res,sqrt(2.0)*mu2,eta,eta,ifail1,stat1);
      acc_std_status("tmcgeo",ifail1,stat1,1,status);

      if (ifail1[0]<0)
      {
         print_status("tmcgeo",ifail1,stat1);
         error_root(1,1,"rwtm2eo [rwtmeo.c]","CGNE solver failed "
                    "(mu = %.2e, parameter set no %d)",sqrt(2.0)*mu2,isp);
      }

      if (mu1>0.0)
         lnr1=norm_square_dble(VOLUME_TRD/2,3,phi);
      else
      {
         lnr1.q[0]=0.0;
         lnr1.q[1]=0.0;
      ...
      lnr2=spinor_prod_re_dble(VOLUME_TRD/2,3,eta,phi);
   }
   else if (sp.solver==SAP_GCR)
   {
      sap=sap_parms();
      ...
      {
         print_status("sap_gcr",ifail0,stat0);
         error_root(1,1,"rwtm2eo [rwtmeo.c]","SAP_GCR solver failed "
                    "(mu = %.2e, parameter set no %d)",mu1,isp);
      }
      ...
      if (mu1>0.0)
      {
         ...
         {
            print_status("sap_gcr",ifail0,stat0);
            error_root(1,2,"rwtm2eo [rwtmeo.c]","SAP_GCR solver failed "
                       "(mu = %.2e, parameter set no %d)",mu1,isp);
         }
      ...
      lnr2=norm_square_dble(VOLUME_TRD/2,3,eta);
   }
   else if (sp.solver==DFL_SAP_GCR)
   {
      sap=sap_parms();
      ...
      if ((ifail0[0]<-2)||(ifail0[1]<0))
      {
         error_root(1,1,"rwtm2eo [rwtmeo.c]","DFL_SAP_GCR solver failed "
                    "(mu = %.2e, parameter set no %d)",mu1,isp);
      }
      ...
      if (mu1>0.0)
      {
         ...
         {
            print_status("dfl_sap_gcr2",ifail0,stat0);
            error_root(1,2,"rwtm2eo [rwtmeo.c]","DFL_SAP_GCR solver failed "
                       "(mu = %.2e, parameter set no %d",mu1,isp);
         }
