...
*                 (Dw^dag*Dw+mu0^2)*psi=phi. The feature can be disabled
*                 by setting icr=0.
*
*   icom          The action returned by the programs setpf2() and
*                 action2() is summed over all MPI processes if the
...
* equation with twisted mass mu0. In the case of rotpf2() the solver first
* used is the one for mass mu1 and the solver for mass mu0 is used only if
* icr>0. In force2() the specified solver solves the Dirac equation twice.
*
...
static char *program[2]={"rotpf2","setpf2"};
...
qflt setpf2(double mu0,double mu1,int ipf,int isp1,int icom,int *status)
{
   ...
   random_sd(VOLUME_TRD,2,psi,1.0);
   ...
   if (sp.solver==CGNE)
   {
      tmcg(sp.nmx,sp.istop,sp.res,mu1,psi,psi,ifail0,stat0);
      acc_std_status("tmcg",ifail0,stat0,0,status);

      if (ifail0[0]<0)
      {
         print_status("tmcg",ifail0,stat0);
         error_root(1,1,"setpf2 [force2.c]","CGNE solver failed "
                    "(mu = %.4e, parameter set no %d)",mu1,isp1);
      }

      rsd=reserve_wsd(1);
      chi=rsd[0];
      assign_sd2sd(VOLUME_TRD,2,psi,chi);
      Dw_dble(-mu1,chi,psi);
   ...
   else if ((sp.solver==SAP_GCR)||(sp.solver==DFL_SAP_GCR))
   {
      ...
      {
         sap_gcr(sp.nkv,sp.nmx,sp.istop,sp.res,mu1,psi,psi,ifail0,stat0);
         acc_std_status("sap_gcr",ifail0,stat0,0,status);

         if (ifail0[0]<0)
         {
            print_status("sap_gcr",ifail0,stat0);
            error_root(1,1,"setpf2 [force2.c]","SAP_GCR solver failed "
                       "(mu = %.4e, parameter set no %d)",mu1,isp1);
         }
      }
      else
      {
         dfl_sap_gcr2(sp.nkv,sp.nmx,sp.istop,sp.res,mu1,psi,psi,ifail0,stat0);
         acc_std_status("dfl_sap_gcr2",ifail0,stat0,0,status);

         if ((ifail0[0]<-2)||(ifail0[1]<0))
         {
            print_status("dfl_sap_gcr2",ifail0,stat0);
            error_root(1,1,"setpf2 [force2.c]","DFL_SAP_GCR solver failed "
                       "(mu = %.4e, parameter set no %d)",mu1,isp1);
         }
      }
   }
   ...
   z.re=0.0;
   z.im=mu0-mu1;
   mulc_spinor_add_dble(VOLUME_TRD,2,phi,psi,z);
            ...
            double c1,double c2,int *status)
{
   ...
   combine_spinor_dble(VOLUME_TRD,2,phi,eta,c1,c2);
   sp=solver_parms(isp1);
   ...
   if (sp.solver==CGNE)
   {
      tmcg(sp.nmx,sp.istop,sp.res,mu1,eta,psi,ifail0,stat0);
      acc_std_status("tmcg",ifail0,stat0,0,status);

      if (ifail0[0]<0)
      {
         print_status("tmcg",ifail0,stat0);
         error_root(1,1,"rotpf2 [force2.c]","CGNE solver failed "
                    "(mu = %.4e, parameter set no %d)",mu1,isp1);
      }

      rsd=reserve_wsd(1);
      rho=rsd[0];
      assign_sd2sd(VOLUME_TRD,2,psi,rho);
      Dw_dble(-mu1,rho,psi);
      mulg5_dble(VOLUME_TRD,2,psi);
      release_wsd();
   }
   else if ((sp.solver==SAP_GCR)||(sp.solver==DFL_SAP_GCR))
   {
      ...
      if (sp.solver==SAP_GCR)
      {
         sap_gcr(sp.nkv,sp.nmx,sp.istop,sp.res,mu1,eta,psi,ifail0,stat0);
         acc_std_status("sap_gcr",ifail0,stat0,0,status);

         if (ifail0[0]<0)
         {
            print_status("sap_gcr",ifail0,stat0);
            error_root(1,1,"rotpf2 [force2.c]","SAP_GCR solver failed "
                       "(mu = %.4e, parameter set no %d)",mu1,isp1);
         }
      }
      else
      {
         dfl_sap_gcr2(sp.nkv,sp.nmx,sp.istop,sp.res,mu1,eta,psi,ifail0,stat0);
         acc_std_status("dfl_sap_gcr2",ifail0,stat0,0,status);

         if ((ifail0[0]<-2)||(ifail0[1]<0))
         {
            print_status("dfl_sap_gcr2",ifail0,stat0);
            error_root(1,1,"rotpf2 [force2.c]","DFL_SAP_GCR solver failed "
                       "(mu = %.4e, parameter set no %d)",mu1,isp1);
         }
      }
   }
   ...
   if (get_chrono(icr,psi))
   {
      ...
      if (sp.solver==CGNE)
      {
         ...
         tmcg(sp.nmx,sp.istop,sp.res,mu0,eta,psi,ifail0,stat0);
