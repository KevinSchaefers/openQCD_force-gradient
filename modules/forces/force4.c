...
* File force4.c
*
* Copyright (C) 2012-2018, 2020, 2022  Martin Luescher, Stefan Schaefer
*
...
*     to exp(-Spf) and replaces phi by c1*phi+c2*eta (see the notes). No
*     use is made of the chronological solver stack, but the stack is
...
*     by c and added to the molecular-dynamics force field. The program
*     makes use of the chronological solver stack and updates the stack
*     on exit.
*
*   qflt small_det(int icom)
*     Returns the action -ln{det(1e+Doo)}+constant associated with the
*     "small determinant".
*
*   qflt action4(double mu,int ipf,int isw,int isp,int icr,int icom,
*                int *status)
*     Returns the action Spf+Sdet if isw=1 or Spf if isw!=1 (see the
*     notes). The chronological solver stack is used but not updated.
*
...
*                 (Dwhat^dag*Dwhat+mu^2)*psi=phi. The feature can be
*                 disabled by setting icr=0.
*
*   icom          The action returned by the programs setpf4() and
*                 action4() is summed over all MPI processes if the
...
qflt setpf4(double mu,int ipf,int isw,int icom)
{
   ...
   act1=norm_square_dble(VOLUME_TRD/2,2,eta);
   add_qflt(act0.q,act1.q,act0.q);

   ifail0[0]=sw_term(ODD_PTS);
   error_root(ifail0[0]!=0,1,"setpf4 [force4.c]",
...
void rotpf4(double mu,int ipf,int isp,int icr,double c1,double c2,int *status)
{
   ...
   ifail0[0]=sw_term(ODD_PTS);
   error_root(ifail0[0]!=0,1,"rotpf4 [force4.c]",
              "Inversion of the SW term was not safe");

   Dwhat_dble(mu,eta,psi);
   mulg5_dble(VOLUME_TRD/2,2,psi);
   combine_spinor_dble(VOLUME_TRD/2,2,phi,psi,c1,c2);
   ...
   if (get_chrono(icr,psi))
   {
      ...
      if (sp.solver==CGNE)
      {
         ...
         tmcgeo(sp.nmx,sp.istop,sp.res,mu,eta,psi,ifail0,stat0);
         acc_std_status("tmcgeo",ifail0,stat0,1,status);

         if (ifail0[0]<0)
         {
            print_status("tmcgeo",ifail0,stat0);
            error_root(1,1,"rotpf4 [force4.c]","CGNE solver failed "
      ...
      else if ((sp.solver==SAP_GCR)||(sp.solver==DFL_SAP_GCR))
      {
         ...
         {
            sap_gcr(sp.nkv,sp.nmx,sp.istop,sp.res,-mu,eta,eta,
                    ifail0,stat0);
            ...
            if (ifail0[0]<0)
            {
               ...
               error_root(1,1,"rotpf4 [force4.c]","SAP_GCR solver failed "
                          "(mu = %.4e, parameter set no %d)",mu,isp);
            }
         }
         else
         {
            dfl_sap_gcr2(sp.nkv,sp.nmx,sp.istop,sp.res,-mu,eta,eta,
                         ifail0,stat0);
            ...
            if ((ifail0[0]<-2)||(ifail0[1]<0))
            {
               ...
               error_root(1,1,"rotpf4 [force4.c]","DFL_SAP_GCR solver failed "
                          "(mu = %.4e, parameter set no %d)",mu,isp);
            }
         }

         combine_spinor_dble(VOLUME_TRD/2,2,psi,eta,c1,c2);
      }
      else
...
void force4(double mu,int ipf,int isw,int isp,int icr,double c,int *status)
{
   ...
   if (isw==1)
   {
      ifail0[0]=add_det2xt(2.0,ODD_PTS);
      error_root(ifail0[0]!=0,1,"force4 [force4.c]",
                 "Inversion of the SW term was not safe");
   }
   ...
   if (sp.solver==CGNE)
   {
      if (get_chrono(icr,chi))
      {
         ...
         if (sp.istop)
         {
            res0=unorm_dble(VOLUME_TRD/2,3,phi);
            res1=unorm_dble(VOLUME_TRD/2,3,rho)/res0;
         }
         else
         {
            rqsm=norm_square_dble(VOLUME_TRD/2,3,phi);
            res0=rqsm.q[0];
            rqsm=norm_square_dble(VOLUME_TRD/2,3,rho);
         ...
         if (res1<1.0)
         {
            if (res1>sp.res)
            {
               tmcgeo(sp.nmx,sp.istop,sp.res/res1,mu,rho,psi,ifail0,stat0);
               mulr_spinor_add_dble(VOLUME_TRD/2,2,chi,psi,-1.0);
            }
         }
         else
         {
            assign_sd2sd(VOLUME_TRD/2,2,phi,psi);
            tmcgeo(sp.nmx,sp.istop,sp.res,mu,psi,chi,ifail0,stat0);
         }
      ...
      {
         print_status("tmcgeo",ifail0,stat0);
         error_root(1,1,"force4 [force4.c]","CGNE solver failed "
                    "(mu = %.4e, parameter set no %d)",mu,isp);
      }
   ...
   else if ((sp.solver==SAP_GCR)||(sp.solver==DFL_SAP_GCR))
   {
      ...
      if (get_chrono(icr,chi))
      {
         ...
         if (res1<1.0)
         {
            ...
            {
               mulg5_dble(VOLUME_TRD/2,2,rho);
               set_sd2zero(VOLUME_TRD/2,2,rho+(VOLUME/2));

               if (sp.solver==SAP_GCR)
                  sap_gcr(sp.nkv,sp.nmx,sp.istop,sp.res/res1,mu,
                          rho,eta,ifail0,stat0);
               else
               ...
               if (sp.istop)
               {
                  res0=unorm_dble(VOLUME_TRD/2,3,psi);
                  res1=unorm_dble(VOLUME_TRD/2,3,eta)/res0;
               }
               else
               {
                  rqsm=norm_square_dble(VOLUME_TRD/2,3,psi);
                  res0=rqsm.q[0];
               ...
               {
                  assign_sd2sd(VOLUME_TRD/2,2,psi,eta);
                  mulg5_dble(VOLUME_TRD/2,2,eta);
                  set_sd2zero(VOLUME_TRD/2,2,eta+(VOLUME/2));

                  if (sp.solver==SAP_GCR)
                     sap_gcr(sp.nkv,sp.nmx,sp.istop,sp.res,-mu,
                             eta,chi,ifail1,stat1);
                  else
      ...
      if (sp.solver==SAP_GCR)
      {
         ...
         if ((ifail0[0]<0)||(ifail1[0]<0))
         {
            ...
            error_root(1,1,"force4 [force4.c]","SAP_GCR solver failed "
                       "(mu = %.4e, parameter set no %d)",mu,isp);
         }
      }
      else
      {
         ...
         if (((ifail0[0]<-2)||(ifail0[1]<0))||((ifail1[0]<-2)||(ifail1[1]<0)))
         {
            ...
            error_root(1,1,"force4 [force4.c]","DFL_SAP_GCR solver failed "
                       "(mu = %.4e, parameter set no %d)",mu,isp);
         }
      }
...
qflt action4(double mu,int ipf,int isw,int isp,int icr,int icom,int *status)
{
   ...
   if (sp.solver==CGNE)
   {
      if (get_chrono(icr,chi))
      {
         ...
         ifail0[0]=sw_term(ODD_PTS);
         error_root(ifail0[0]!=0,1,"action4 [force4.c]",
                    "Inversion of the SW term was not safe");
         ...
         mulg5_dble(VOLUME_TRD/2,2,psi);
         mulr_spinor_add_dble(VOLUME_TRD/2,2,psi,phi,-1.0);
         ...
         {
            res0=unorm_dble(VOLUME_TRD/2,3,phi);
            res1=unorm_dble(VOLUME_TRD/2,3,psi)/res0;
         }
         else
         {
            rqsm=norm_square_dble(VOLUME_TRD/2,3,phi);
         ...
         if (res1<1.0)
         {
            if (res1>sp.res)
            {
               tmcgeo(sp.nmx,sp.istop,sp.res/res1,mu,psi,psi,ifail0,stat0);
               mulr_spinor_add_dble(VOLUME_TRD/2,2,chi,psi,-1.0);
            }
         }
         else
      ...
      {
         print_status("tmcgeo",ifail0,stat0);
         error_root(1,1,"action4 [force4.c]","CGNE solver failed "
                    "(mu = %.4e, parameter set no %d)",mu,isp);
      }
   ...
   else if ((sp.solver==SAP_GCR)||(sp.solver==DFL_SAP_GCR))
   {
      ...
      if (get_chrono(icr,chi))
      {
         ...
         {
            res0=unorm_dble(VOLUME_TRD/2,3,phi);
            res1=unorm_dble(VOLUME_TRD/2,3,chi)/res0;
         }
         else
         {
            rqsm=norm_square_dble(VOLUME_TRD/2,3,phi);
            res0=rqsm.q[0];
            rqsm=norm_square_dble(VOLUME_TRD/2,3,chi);
            res1=sqrt(rqsm.q[0]/res0);
         }
         ...
         if (res1<1.0)
         {
            if (res1>sp.res)
            {
               ...
               if (sp.solver==SAP_GCR)
                  sap_gcr(sp.nkv,sp.nmx,sp.istop,sp.res/res1,mu,chi,chi,
                          ifail0,stat0);
               else
                  dfl_sap_gcr2(sp.nkv,sp.nmx,sp.istop,sp.res/res1,mu,chi,chi,
                               ifail0,stat0);

               mulr_spinor_add_dble(VOLUME_TRD/2,2,psi,chi,-1.0);
            }
         }
         else
         {
      ...
      if (sp.solver==SAP_GCR)
      {
         ...
         {
            print_status("sap_gcr",ifail0,stat0);
            error_root(1,1,"action4 [force4.c]","SAP_GCR solver failed "
                       "(mu = %.4e, parameter set no %d)",mu,isp);
         }
      }
      else
      {
         ...
         {
            print_status("dfl_sap_gcr2",ifail0,stat0);
            error_root(1,1,"action4 [force4.c]","DFL_SAP_GCR solver failed "
                       "(mu = %.4e, parameter set no %d)",mu,isp);
         }
      }
   }
