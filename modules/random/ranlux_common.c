...
*   stdlong_t (*state)[4]        Current state vectors and carry bits.
*
...
* type stdlong_t is defined in utils.h and is guaranteed to be an integer
* data type of size 8.
*
...
static const stdlong_t base=(stdlong_t)(0x1000000000000);
static const stdlong_t mask=(stdlong_t)(0xffffffffffff);
...
void rlx_init(rlx_state_t *s,int seed,int flag)
{
   ...
   for (i=0;i<4;i++)
   {
      for (k=0;k<24;k++)
      {
         ...
         if (k&0x1)
            state[k/2][i]+=((stdlong_t)(ix)<<24);
         else
            state[k/2][i]=(stdlong_t)(ix);
      }
   }

   (*s).state[12][0]=(stdlong_t)(0);
   (*s).state[12][1]=(stdlong_t)(0);
   (*s).state[12][2]=(stdlong_t)(0);
   (*s).state[12][3]=(stdlong_t)(0);

   (*s).state[13][0]=(stdlong_t)(0);
   (*s).state[13][1]=(stdlong_t)(0);
   (*s).state[13][2]=(stdlong_t)(0);
   (*s).state[13][3]=(stdlong_t)(0);
...
{
   int k;
   stdlong_t lmask,(*state)[4];

   lmask=(stdlong_t)(0xffffff);
   state=(*s).state;
...
void rlx_set_state(int *is,rlx_state_t *s)
{
   ...
   for (k=0;k<12;k++)
   {
      (*state)[0]=(stdlong_t)(is[0]);
      (*state)[1]=(stdlong_t)(is[1]);
      (*state)[2]=(stdlong_t)(is[2]);
      (*state)[3]=(stdlong_t)(is[3]);

      (*state)[0]+=((stdlong_t)(is[4])<<24);
      (*state)[1]+=((stdlong_t)(is[5])<<24);
      (*state)[2]+=((stdlong_t)(is[6])<<24);
      (*state)[3]+=((stdlong_t)(is[7])<<24);
...
#define _avx_load_state(reg1,reg2) \
...
{
   int pr,ir,k;
   stdlong_t shift,(*pmin)[4],(*pmax)[4],(*pi)[4],(*pj)[4];
   ...
   _avx_load_state(%%ymm0,xmm0);
   _avx_load_state(%%ymm1,xmm1);
   _avx_load_state(%%ymm2,xmm2);
   _avx_load_state(%%ymm3,xmm3);
   _avx_load_state(%%ymm4,xmm4);
   _avx_load_state(%%ymm5,xmm5);
   _avx_load_state(%%ymm6,xmm6);
   _avx_load_state(%%ymm7,xmm7);
   _avx_load_state(%%ymm8,xmm8);
   _avx_load_state(%%ymm9,xmm9);
   _avx_load_state(%%ymm10,xmm10);
   _avx_load_state(%%ymm11,xmm11);
   ...
   _avx_store_state(%%ymm0);
   _avx_store_state(%%ymm1);
   _avx_store_state(%%ymm2);
   _avx_store_state(%%ymm3);
   _avx_store_state(%%ymm4);
   _avx_store_state(%%ymm5);
   _avx_store_state(%%ymm6);
   _avx_store_state(%%ymm7);
   _avx_store_state(%%ymm8);
   _avx_store_state(%%ymm9);
   _avx_store_state(%%ymm10);
   _avx_store_state(%%ymm11);
...
{
   int pr,ir,k;
   stdlong_t d[4],(*pmin)[4],(*pmax)[4],(*pi)[4],(*pj)[4];
...
{
   int k;
   stdlong_t sexp,(*state)[4];

   sexp=(stdlong_t)(0x4030000000000000);
   state=(*s).state;
