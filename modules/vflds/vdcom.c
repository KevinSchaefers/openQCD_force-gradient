...
*
*   void cpvd_int_bnd(int ieo,complex_dble *vd)
...
*
* The fields passed to cpvd_int_bnd() are interpreted as elements of the
...
* boundary.
*
* After calling cpvd_int_bnd(), the boundary values of the field on the
...
*
* The program cpvd_int_bnd() is assumed to be called by the OpenMP master
...
static void set_ofs(void)
{
   ...
   error_root(Ns==0,1,"set_ofs [vdcom.c]",
   ...
   error((a==NULL)||(ofs_int==NULL)||(ofs_bnd==NULL),1,
         "alloc_ofs [vdcom.c]","Unable to allocate auxiliary arrays");
   b=a+NTHREAD;
...
static void alloc_sbufs(void)
{
   ...
   error(w==NULL,1,"alloc_sbufs [vdcom.c]",
...
static void get_vdbnd(int ofs,int vol,complex_dble *vd,complex_dble *sbuf)
{
   int *ip,*im;
   complex_dble *vv,*vm;
...
static void send_buf(int ieo,int ifc,complex_dble *vd)
{
   ...
   {
      if (nsbf)
         MPI_Isend(sbuf,nsbf,MPI_DOUBLE,saddr,tag,MPI_COMM_WORLD,&snd_req);
      if (nrbf)
         MPI_Irecv(rbuf,nrbf,MPI_DOUBLE,raddr,tag,MPI_COMM_WORLD,&rcv_req);
         ...
         for (ifc=0;ifc<8;ifc++)
         {
            ...
            if (sflg[sfc])
            {
               if ((NTHREAD==1)||(k>0))
               {
                  ...
                  get_vdbnd(ofs,vol,vd,sbufs[ifc&0x1]+sof);
               }
