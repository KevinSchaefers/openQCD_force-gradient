...
* File blk_solv.c
*
* Copyright (C) 2005, 2011-2013, 2016, 2021 Martin Luescher, Isabel Campos
*
...
static void scalar_prods(int vol,float *r,complex *z,spinor **s)
{
   spinor *s1,*s2,*sm,*smb;
...
static void linear_cmbs(int vol,float *r,complex *z,spinor **s)
{
   spinor *s0,*s1,*s2,*sm;
...
void blk_mres(int n,float mu,int nmr)
{
   ...
   b=blk_list(SAP_BLOCKS,&nb,&isw)+n;
   vol=(*b).vol;
   s=(*b).s;

   set_s2zero(vol,0,s[0]);

   for (imr=0;imr<nmr;imr++)
   {
      Dw_blk(SAP_BLOCKS,n,mu,1,2);
      scalar_prods(vol,&r,&z,s);

      if (r<(2.0f*FLT_MIN))
         return;

      linear_cmbs(vol,&r,&z,s);
   }
}
...
void blk_mres(int n,float mu,int nmr)
{
   ...
   for (imr=0;imr<nmr;imr++)
   {
      ...
      z.re*=r;
      z.im*=r;
      mulc_spinor_add(vol,0,s[0],s[1],z);

      z.re=-z.re;
      z.im=-z.im;
      mulc_spinor_add(vol,0,s[1],s[2],z);
   }
}
